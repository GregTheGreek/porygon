...
class Project:
    ...
    # --- tilesets / fieldmap --------------------------------------------
    def _fieldmap_constant(self, name: str, default: int) -> int:
        """Read a ``#define <name> <int>`` from include/fieldmap.h (else default)."""
        fieldmap = self.root / "include" / "fieldmap.h"
        if fieldmap.exists():
            import re
            m = re.search(rf"#define\s+{re.escape(name)}\s+(\d+)", fieldmap.read_text())
            if m:
    ...
    def tiles_per_metatile(self) -> int:
        ...
        return self._fieldmap_constant("NUM_TILES_PER_METATILE", 8)

    def num_tiles_in_primary(self) -> int:
        """Tile indices below this reference the primary tileset's tiles.png."""
        return self._fieldmap_constant("NUM_TILES_IN_PRIMARY", 512)

    def num_metatiles_in_primary(self) -> int:
        """Secondary metatile ids start at this offset in the combined id space."""
        return self._fieldmap_constant("NUM_METATILES_IN_PRIMARY", 512)

    def num_pals_in_primary(self) -> int:
        """Palette slots below this index belong to the primary tileset."""
        return self._fieldmap_constant("NUM_PALS_IN_PRIMARY", 6)
    ...
    def find_tileset_dir(self, folder: str) -> tuple[Path, bool]:
        """Locate a tileset folder under primary/ or secondary/.

        Returns (path, is_secondary). Raises if it exists in neither.
        """
        for secondary in (False, True):
            d = self.tileset_dir(folder, secondary=secondary)
            if (d / "metatiles.bin").exists():
                return d, secondary
        raise ProjectError(
            f"tileset folder {folder!r} not found under data/tilesets/primary or /secondary"
        )

    def list_tilesets(self) -> list[dict]:
        """List on-disk tilesets (folder, kind, metatile count) for the caller to pick from."""
        out = []
        tpm = self.tiles_per_metatile()
        for kind in ("primary", "secondary"):
            base = self.root / "data" / "tilesets" / kind
            if not base.is_dir():
                continue
            for d in sorted(base.iterdir()):
                mt = d / "metatiles.bin"
                if not mt.is_dir() and mt.exists():
                    count = mt.stat().st_size // (tpm * 2)
                    out.append({"folder": d.name, "kind": kind, "metatile_count": count})
        return out
    ...
    def _write_map_groups(self, data: dict) -> Path:
        path = self._map_groups_path()
    ...
    def add_map(self, map_id: str, name: str, layout_id: str, *, group: Optional[str] = None,
                ...
                allow_escaping: bool = False, allow_running: bool = True,
                show_map_name: bool = True,
                battle_scene: str = "MAP_BATTLE_SCENE_NORMAL") -> Path:
        ...
        if map_dir.exists():
            raise ProjectError(f"map directory {map_dir} already exists")
        ...
        (map_dir / "map.json").write_text(json.dumps(data, indent=2) + "\n")

        # Register the map's folder name under its group (the build reads these arrays).
        if name not in groups[group]:
            groups[group].append(name)
            self._write_map_groups(groups)
